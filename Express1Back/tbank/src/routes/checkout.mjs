...
import { initTbankPayment } from "../tbank.mjs";
...
export function makeCheckoutRouter({ db }) {
  ...
  r.post("/checkout/create", async (req, res) => {
    ...
    const result = await withTx(db, async () => {
      ...
      const description = getCheckoutDescription(p);

      let initResult;
      try {
        initResult = await initTbankPayment({
        ...
        return {
          _error: {
            status: 502,
            message: String(e?.message || "Failed to create payment in T-Bank")
          }
        };
      }

      const payUrl = String(initResult.paymentUrl);
      ...
      // Create pending sale so notification can find it by OrderId/InvId.
